...
const char *led_shell_mode   = "500,500,";      /* 1Hz  ��˸ */
const char *led_upgrade_mode = "50,50,";        /* 10Hz ��˸ */
const char *led_off_mode     = "0,100,";        /* ���� */
const char *led_on_mode      = "100,0,";        /* ���� */
...
int rt_fota_upgrade(const char *part_name)
{
    ...
    /* �п�������ֵ�����,��ΪAES��Ҫ���16�ֽ�����,�����Ľ��ܽ�ѹֵ�Ĵ������������Ǵ��ڵ���raw_size */
    /* �ȽϺõķ�������һ��У��,Ŀǰ��������HASH_CODE�㷨��֪�� */
...
static int rt_fota_start_application(void)
{
    int fota_res = RT_FOTA_NO_ERR;
    const struct fal_partition *part;
    static rt_uint32_t app_addr;
    static application_entry_t app_entry;

    part = fal_partition_find(RT_FOTA_APP_PART_NAME);//APP这部分
    if (part == RT_NULL)
    {
    ...
    app_addr = part->offset + 0x80000000;



    if (((*(__IO uint32_t *)app_addr) & 0xFFFFFFFF) != 0x0A4D5048UL)  //检查首地址是不是合法的
    {
        LOG_I("地址不合法.");
        fota_res = RT_FOTA_GENERAL_ERR;
        goto __exit_start_application;
...
#if 0
    if (((*(__IO uint32_t *)(app_addr + 4)) & 0xff000000) != 0x80000000)
    {
    ...
    fencei();
    app_entry   = (application_entry_t)(app_addr);
    app_entry();
    while(1);
...
void rt_fota_thread_entry(void *arg)
{
    ...
    fota_err = rt_fota_part_fw_verify(RT_FOTA_FM_PART_NAME);//download 分区
    if (fota_err != RT_FOTA_NO_ERR)
    ...
    fota_err = rt_fota_upgrade(RT_FOTA_FM_PART_NAME); //更新到flash分区去
    if (fota_err != RT_FOTA_NO_ERR)
        goto __exit_boot_entry;

    /* Update new application verison in RBL file of firmware partition */
    fota_err = rt_fota_copy_version(RT_FOTA_FM_PART_NAME);  //拷贝版本
    if (fota_err != RT_FOTA_NO_ERR)
        goto __exit_boot_entry;

__exit_boot_entry:
    /* Implement application */
    rt_fota_start_application();//启动应用
